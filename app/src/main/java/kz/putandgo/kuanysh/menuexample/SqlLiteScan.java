...
public class SqlLiteScan {
    private static final String DB_NAME = "Scan";
    private static final int DB_VERSION = 2;
    private static final String DB_TABLE = "userScan";
    ...
    private SqlLiteScan.DBHelper mDBHelper;
    private SQLiteDatabase mDB;
    public SqlLiteScan(Context ctx) {
        mCtx = ctx;
    }
    ...
    public void open() {
        ...
        mDBHelper = new SqlLiteScan.DBHelper(mCtx, DB_NAME, null, DB_VERSION);
    ...
    public String brutto(String marketId){
        String selectQuery = "SELECT * FROM " + DB_TABLE + " WHERE "+COLUMN_MARKET_ID+"="+marketId;
        Cursor cursor = mDB.rawQuery(selectQuery, null);
        double i=0;
        if (cursor.moveToFirst()) {
            do {
                ...
                i=i+(cursor.getInt(8)*cursor.getInt(18));
            } while (cursor.moveToNext());
        }
        return i+"";
    }
